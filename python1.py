...
Antes de correr cualquier sección, definir:
    X = ...   # array de entrada  (n_samples, n_features)
    y = ...   # array objetivo    (n_samples,)
"""
...
ax.plot(v_rng, modelo.predict(v_rng), color='crimson', lw=2,
        label=f'Lineal  ($R^2={r2_score(y_test, y_pred):.3f}$)')
ax.set_xlabel('X'); ax.set_ylabel('y')
ax.set_title('Regresión Lineal'); ax.legend(); ax.grid(True, alpha=0.35)
...
fig, ax = plt.subplots(figsize=(8, 5))
ax.scatter(X, y, color='steelblue', alpha=0.6, s=40, label='Datos experimentales')
ax.plot(v_rng, mejor_modelo.predict(v_rng), color='darkorange', lw=2,
        label=f'Polinomio grado {mejor_grado}  ($R^2={mejorR2:.3f}$)')
ax.set_xlabel('X'); ax.set_ylabel('y')
ax.set_title(f'Regresión Polinomial (grado {mejor_grado})'); ax.legend(); ax.grid(True, alpha=0.35)
plt.tight_layout(); plt.show()
...
axes[0].plot(v_rng, mejor_modelo.predict(v_rng), color='forestgreen', lw=2,
             label=f'Árbol depth={mejor_prof}  ($R^2={mejorR2:.3f}$)')
axes[0].set_xlabel('X'); axes[0].set_ylabel('y')
axes[0].set_title('Árbol de Decisión'); axes[0].legend(); axes[0].grid(True, alpha=0.35)
...
fig, ax = plt.subplots(figsize=(8, 5))
ax.scatter(X, y, color='steelblue', alpha=0.6, s=40, label='Datos experimentales')
ax.plot(v_rng, mejor_modelo.predict(v_rng), color='darkorchid', lw=2,
        label=f'KNN k={mejor_k}  ($R^2={mejorR2:.3f}$)')
ax.set_xlabel('X'); ax.set_ylabel('y')
ax.set_title(f'KNN Regressor (k={mejor_k})'); ax.legend(); ax.grid(True, alpha=0.35)
plt.tight_layout(); plt.show()
...
for k in ['linear', 'poly', 'rbf']:
    ...
    ax.plot(v_rng, pipe_k.predict(v_rng), lw=1.8, color=colores_k[k],
            ls='-' if k == mejor_kernel else '--',
            alpha=1.0 if k == mejor_kernel else 0.5,
            label=f'SVR {k}' + ('  ← mejor' if k == mejor_kernel else ''))
ax.set_xlabel('X'); ax.set_ylabel('y')
ax.set_title('SVR – Comparación de kernels'); ax.legend(); ax.grid(True, alpha=0.35)
plt.tight_layout(); plt.show()
...
preds_arb   = np.array([t.predict(v_rng) for t in mejor_modelo.estimators_])
std_rng     = preds_arb.std(axis=0)

fig, ax = plt.subplots(figsize=(8, 5))
ax.scatter(X, y, color='steelblue', alpha=0.6, s=40, label='Datos experimentales')
ax.plot(v_rng, y_rng, color='saddlebrown', lw=2,
        label=f'Random Forest n={mejor_n}  ($R^2={mejorR2:.3f}$)')
ax.fill_between(v_rng.ravel(), y_rng - std_rng, y_rng + std_rng,
                alpha=0.2, color='saddlebrown', label='±1σ entre árboles')
ax.set_xlabel('X'); ax.set_ylabel('y')
ax.set_title(f'Random Forest (n={mejor_n})'); ax.legend(); ax.grid(True, alpha=0.35)
plt.tight_layout(); plt.show()
...
axes[0].plot(v_rng, mejor_modelo.predict(v_rng), color='darkgreen', lw=2,
             label=f'GBR lr={mejor_lr}  ($R^2={mejorR2:.3f}$)')
axes[0].set_xlabel('X'); axes[0].set_ylabel('y')
axes[0].set_title('Gradient Boosting'); axes[0].legend(); axes[0].grid(True, alpha=0.35)
...
axes[0].plot(v_rng, mejor_modelo.predict(v_rng), color='tomato', lw=2,
             label=f'MLP ReLU {mejor_arq}  ($R^2={mejorR2:.3f}$)')
axes[0].set_xlabel('X'); axes[0].set_ylabel('y')
axes[0].set_title('MLP ReLU'); axes[0].legend(); axes[0].grid(True, alpha=0.35)
axes[1].plot(mejor_modelo.named_steps['mlp'].loss_curve_, color='tomato', lw=1.8)
axes[1].set_xlabel('Época'); axes[1].set_ylabel('Loss')
axes[1].set_title('Curva de pérdida'); axes[1].grid(True, alpha=0.35)
plt.tight_layout(); plt.show()
...
axes[0].plot(v_rng, mejor_modelo.predict(v_rng), color='mediumseagreen', lw=2,
             label=f'MLP tanh {mejor_arq}  ($R^2={mejorR2:.3f}$)')
axes[0].set_xlabel('X'); axes[0].set_ylabel('y')
axes[0].set_title('MLP tanh'); axes[0].legend(); axes[0].grid(True, alpha=0.35)
...
axes[0].plot(v_rng, mejor_modelo.predict(v_rng), color='goldenrod', lw=2,
             label=f'MLP sigmoid {mejor_arq}  ($R^2={mejorR2:.3f}$)')
axes[0].set_xlabel('X'); axes[0].set_ylabel('y')
axes[0].set_title('MLP sigmoid'); axes[0].legend(); axes[0].grid(True, alpha=0.35)
...
print(f"  Frontera: X = {v_frontera:.4f}")
...
for clase in np.unique(y):
    m = y.ravel() == clase
    ax1.scatter(X[m], np.zeros(m.sum()) if X.shape[1] == 1 else X[m, 1],
                alpha=0.7, s=45, label=f'Clase {int(clase)}')
ax1.set_xlabel('X'); ax1.set_title('Figura 1 – Datos por clase')
ax1.legend(); ax1.grid(True, alpha=0.35)
ax2.plot(v_rng, proba, color='purple', lw=2.5, label='$P(clase=1|X)$')
ax2.axvline(v_frontera, color='black', ls='--', lw=1.5,
            label=f'Frontera = {v_frontera:.3f}')
ax2.axhline(0.5, color='gray', ls=':', lw=1)
ax2.fill_between(v_rng.ravel(), proba, 0.5, where=(proba >= 0.5), alpha=0.12, color='orangered')
ax2.fill_between(v_rng.ravel(), proba, 0.5, where=(proba <  0.5), alpha=0.12, color='steelblue')
ax2.set_xlabel('X'); ax2.set_ylabel('Probabilidad predicha')
ax2.set_title('Figura 2 – Probabilidad y frontera'); ax2.set_ylim(-0.05, 1.05)
...
cambios    = np.where(np.diff(mejor_modelo.predict(v_rng)))[0]
v_frontera = v_rng[cambios[0], 0] if len(cambios) > 0 else None

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(13, 5))
for clase in np.unique(y):
    m = y.ravel() == clase
    ax1.scatter(X[m], np.zeros(m.sum()), alpha=0.7, s=45, label=f'Clase {int(clase)}')
ax1.set_xlabel('X'); ax1.set_title('Figura 1 – Datos por clase')
ax1.legend(); ax1.grid(True, alpha=0.35)
ax2.plot(v_rng, proba, color='forestgreen', lw=2.5, label='$P(clase=1|X)$')
...
ax1.set_xlabel('X'); ax1.set_title('Figura 1 – Datos por clase')
ax1.legend(); ax1.grid(True, alpha=0.35)
ax2.plot(v_rng, proba, color='darkorchid', lw=2.5, label=f'$P(clase=1|X)$  k={mejor_k}')
if v_frontera is not None:
    ax2.axvline(v_frontera, color='black', ls='--', lw=1.5,
                label=f'Frontera ≈ {v_frontera:.3f}')
ax2.axhline(0.5, color='gray', ls=':', lw=1)
...
ax1.set_xlabel('X'); ax1.set_title('Figura 1 – Datos por clase')
ax1.legend(); ax1.grid(True, alpha=0.35)
ax2.plot(v_rng, proba, color=colores_k[mejor_kernel], lw=2.5,
         label=f'$P(clase=1|X)$  kernel={mejor_kernel}')
if v_frontera is not None:
    ax2.axvline(v_frontera, color='black', ls='--', lw=1.5,
                label=f'Frontera ≈ {v_frontera:.3f}')
ax2.axhline(0.5, color='gray', ls=':', lw=1)
...
ax1.set_xlabel('X'); ax1.set_title('Figura 1 – Datos por clase')
ax1.legend(); ax1.grid(True, alpha=0.35)
ax2.plot(v_rng, proba, color='saddlebrown', lw=2.5, label='$P(clase=1|X)$')
if v_frontera is not None:
    ax2.axvline(v_frontera, color='black', ls='--', lw=1.5,
                label=f'Frontera ≈ {v_frontera:.3f}')
ax2.axhline(0.5, color='gray', ls=':', lw=1)
...
ax1.set_xlabel('X'); ax1.set_title('Figura 1 – Datos por clase')
ax1.legend(); ax1.grid(True, alpha=0.35)
ax2.plot(v_rng, proba, color='darkgreen', lw=2.5, label='$P(clase=1|X)$')
if v_frontera is not None:
    ax2.axvline(v_frontera, color='black', ls='--', lw=1.5,
                label=f'Frontera ≈ {v_frontera:.3f}')
ax2.axhline(0.5, color='gray', ls=':', lw=1)
...
ax1.set_xlabel('X'); ax1.set_title('Figura 1 – Datos por clase')
ax1.legend(); ax1.grid(True, alpha=0.35)
ax2.plot(v_rng, proba, color='tomato', lw=2.5, label=f'$P(clase=1|X)$  ReLU')
if v_frontera is not None:
    ax2.axvline(v_frontera, color='black', ls='--', lw=1.5,
                label=f'Frontera ≈ {v_frontera:.3f}')
ax2.axhline(0.5, color='gray', ls=':', lw=1)
...
ax1.set_xlabel('X'); ax1.set_title('Figura 1 – Datos por clase')
ax1.legend(); ax1.grid(True, alpha=0.35)
ax2.plot(v_rng, proba, color='mediumseagreen', lw=2.5, label='$P(clase=1|X)$  tanh')
if v_frontera is not None:
    ax2.axvline(v_frontera, color='black', ls='--', lw=1.5,
                label=f'Frontera ≈ {v_frontera:.3f}')
ax2.axhline(0.5, color='gray', ls=':', lw=1)
...
ax1.set_xlabel('X'); ax1.set_title('Figura 1 – Datos por clase')
ax1.legend(); ax1.grid(True, alpha=0.35)
ax2.plot(v_rng, proba, color='goldenrod', lw=2.5, label='$P(clase=1|X)$  sigmoid')
if v_frontera is not None:
    ax2.axvline(v_frontera, color='black', ls='--', lw=1.5,
                label=f'Frontera ≈ {v_frontera:.3f}')
ax2.axhline(0.5, color='gray', ls=':', lw=1)
...
for i, clase in enumerate(modelo.classes_):
    print(f"  Clase {int(clase)}:  μ={modelo.theta_[i, 0]:.4f}  |  σ²={modelo.var_[i, 0]:.6f}")
...
ax1.set_xlabel('X'); ax1.set_title('Figura 1 – Datos por clase')
ax1.legend(); ax1.grid(True, alpha=0.35)
ax2.plot(v_rng, proba, color='teal', lw=2.5, label='$P(clase=1|X)$')
if v_frontera is not None:
    ax2.axvline(v_frontera, color='black', ls='--', lw=1.5,
                label=f'Frontera ≈ {v_frontera:.3f}')
ax2.axhline(0.5, color='gray', ls=':', lw=1)
